...
>
> **4ᵉ passe — plan d'exécution (✅ 2026-07-15) :** (E1) **exécution agentique réelle** contre un vrai `opencode serve` (stub LLM keyless), round-trip d'outil live — voir §4.2 ; (E2) **rotation JWKS live** implémentée (rechargement /5 min, `TASK_JWT_JWKS_RELOAD_SECONDS`, fail-safe : un refresh KO conserve les dernières clés valides) ; (E3) **seam enveloppe KMS** (`KmsProvider`, `LocalKmsProvider` défaut, vrai KMS injectable, format at-rest inchangé) ; (E4) **taint Redis prouvé E2E** cross-langage. *Reste, hors-boîte :* gVisor (`runsc` absent), round-trip OIDC réel (Entra/Slack), client KMS cloud réel, egress K8s.
>
> **5ᵉ passe — événementiel, connecteurs, orchestrateur (✅ 2026-07-15) :** (F1) **Gateway `/mcp` réel** ajouté (opencode atteint la vraie Gateway, revue sécu LOW, voir §4.3). (F2) **Webhook ingress §15.8** livré + **durci** (revue sécu : 0 HIGH restant) — signature v0 timestamp-liée / GitHub body+delivery, anti-rejeu fail-closed, dedup-on-success, storm par-org, secret par `(source,org)` avec fan-out scopé, cap body 1 Mio → InboundMessage `channel=webhook, untrusted` → **même pipeline** (ADR-005). (F3) **Enforcement pipeline** : un tour webhook/untrusted est non-interactif (`origin=scheduled`, approbation fail-closed) et **pré-tainté** (egress reclassé, mémoires `untrusted`). (F4) **UI Automatisations** câblée sur `/api/v1/automations` réel (§2.6, champ fabriqué `costPerRun` supprimé). (F5) **Orchestrateur** (§3.7) : FSM `Next()` pur + edges DESTROYED/FAILED→COLD, formule pool-warmer, score bin-packing, file de priorité interactif>planifié (Go, **26 tests**). (F6) **Connecteurs MCP** : `_template` (§14.3, retries/breaker/OTel/troncature, **22 tests**), **Browser** anti-SSRF durci (encodages IP entiers, IPv6-mapped, metadata, rebinding DNS, **71 tests**), **Database** read-only avec write-guard SQL fail-closed (masker chaînes/commentaires, **54 tests**). (F7) **Browser + Database enregistrés dans la Gateway** (§17.6.2) : métadonnées taint déclarées (`browser.*` : `ingests_untrusted` + `egress_class=public` → reclassé sur tour contaminé ; `database.*` : read-only, `egress_class=none`), exposés via `tools/list` (gated `allowed_tools`), reclassification `browser.fetch` sur tour tainté prouvée (**gateway 66 tests**). Les deux connecteurs sont désormais **atteignables de bout en bout** agent → Gateway → outil.
...
**État :** config 🟩 (`sandbox/opencode.json` déclare la Gateway en MCP remote `Bearer {env:TASK_JWT}` + le provider `llm-proxy`, `@ai-sdk/openai-compatible` ; test Go `TestOpenCodeConfigShape`, `opencode_config_test.go`). **✅ Exécution réelle (2026-07-15) :** un 3ᵉ mode `runner.py` (activé par `OPENCODE_SERVER_URL`) pilote un vrai `opencode serve` — `POST /session`, `POST /session/{id}/message`, SSE `GET /event`, `abort`, mapping événements → `AgentEvent` (`text.delta`/`thinking`/`tool.call`/`tool.result`/`done`). LLM = **stub llm-proxy keyless** (`openai_compat.py`, `POST /v1/chat/completions` adossé au `StubBackend`, injecte un `tool_call` puis le texte final). Test d'intégration live (`RUN_OPENCODE_IT=1`) : **1 passed** — vrai opencode + stub, round-trip d'outil réel. **⚠️ Reste :** (a) gVisor indisponible ici (`runsc` absent) → process hôte, pas le runtime durci ; (b) ✅ **Gateway-in-the-loop prouvé (2026-07-15) :** un tour opencode réel appelle `github.search` **à travers la vraie Gateway** (`gw.call()` : vérif JWT → `allowed_tools` → taint → DLP → **audit**), attesté par la **ligne d'audit** `{actor: usr_dev, tool: github.search, status: ok}` + le nom d'outil server-préfixé `mcp-gateway_github_search` (donc MCP remote, pas builtin). Test `test_gateway_e2e.py` (`RUN_GATEWAY_IT=1`) : **1 passed**. **⚠️→✅ Découverte + correction (2026-07-15) :** la Gateway livrée n'exposait **que du REST bespoke** (`POST /v1/tool/call`), **aucun endpoint MCP** — or `opencode.json` pointe sur `.../mcp` (MCP Streamable HTTP), donc en prod opencode ne pouvait **pas** parler à la Gateway. **Corrigé :** endpoint MCP réel `POST /mcp` ajouté à `server.ts` (`mcp.ts` : `initialize`/`tools/list` JWT-gated aux `allowed_tools`/`tools/call`), **déléguant à `gw.call()`** — même chaîne d'auth, aucun vérifieur parallèle. **Revue de sécurité : risque FAIBLE, 0 critique/haut, parité d'auth confirmée** ; 3 durcissements appliqués (cap body 1 Mio + batch 50 = anti-DoS, texte d'erreur d'auth constant, pas de reflet du nom d'outil). Le test E2E pointe désormais sur le **vrai serveur** (`node src/server.ts`), l'adaptateur dev est **supprimé**. gateway **57**, E2E **1 passed**. (c) rotation du `TASK_JWT` par tour = redémarrage opencode ou re-lecture `{file:...}` (noté pour l'Orchestrator).
...
> ✅ **Livré + durci (2026-07-15) :** `POST /webhooks/{source}` (`backend-core/app/webhooks.py`). Revue de sécurité : **0 HIGH restant** après correctifs. Signature **fail-closed** : GitHub `X-Hub-Signature-256` (body) + `X-GitHub-Delivery` **obligatoire** ; autres sources **v0 timestamp-liée** (`v0:ts:body`, ts signé **obligatoire**). **Anti-rejeu ±5 min fail-closed** (ts manquant → rejet, plus de fail-open). **Dedup** par delivery-id ou `sha256(body signé)`, jamais sauté ; **dedup-on-success** (échec de publish → 502 sans consommer la clé → rejouable). **Storm** keyé par `(source, org, automation)` — un flood n'écrase pas les events d'une autre org, et une suppression totale reste rejouable. **Secret par `(source, org)`** + fan-out scopé à l'org (`?org=`) → une signature n'active que les automatisations de **son** tenant. **Cap body 1 Mio** avant tout travail. `InboundMessage{channel:webhook, untrusted:true}` → le pipeline le rend **non-interactif + pré-tainté** (§5.4). Tests : backend-core **103**, prompt-layer **187**.
